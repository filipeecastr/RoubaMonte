...
void distribuiCartaMesa(ListaMesa* l, Pilha *b, int resto) {
...
int main(){
    ...
    baralho bPrincipal[qtdBaralhos];

    for(int j=0; j<qtdBaralhos; j++){
        inicializaBaralho(&bPrincipal[j]);
        embaralhar(&bPrincipal[j]);
        
        for(int k=0; k<52; k++){
            empilhar(pilhaBaralho, &bPrincipal[j].carta[k]);
        }
    }
    ...
    int rest=qtdCartas%jogadores;

    if(rest!=0){
        distribuiCartaMesa(listMesa, pilhaBaralho, rest);
    }
    ...
    do{
        for(cont=0; cont<jogadores; cont++){
            if(listaVazia(p[cont]->l)==0){
                ...
                if(cartaSelecionada<0 || cartaSelecionada>1){
                    printf("Posicao Invalida!\n");
                    cont--;
                }else{
